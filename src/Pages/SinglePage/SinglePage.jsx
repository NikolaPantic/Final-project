...
function SinglePage(props) {
  ...
  useEffect(() => setSingleCandidate(x.candidates.find((e) => e.id == props.match.params.id)), [x.candidates]);

  useEffect(() => {
    setCandReport(x.reports.filter((e) => e.candidateId == props.match.params.id));
  }, [x.reports]);
          ...
          <div className="single-page-container">
            <div className="person-image-and-data">
              <div className="person-image-div">
                <img className="person-image" src={singleCandidate.avatar} alt="candidate" />
              </div>
              <div className="person-data">
                ...
                <div className="person-data-field">
                  ...
                  <h2 className="single-person-data">{new Date(singleCandidate.birthday).toLocaleDateString("en-UK").split("/").join(".")}</h2>
                </div>
                <div className="person-data-field">
                  <p className="single-person-heading">Email:</p>
                  <h2 className="single-person-data">{singleCandidate.email.toLowerCase()}</h2>
                </div>
                <div className="person-data-field">
                  <p className="single-person-heading">Education:</p>
                  <h2 className="single-person-data">{singleCandidate.education}</h2>
                </div>
              </div>
            </div>
                ...
                <table>
                  ...
                  {candReport?.map((e, i) => (
                    <tr key={i}>
                      ...
                      <td className="interview-dates">{new Date(e.interviewDate).toLocaleDateString("en-UK").split("/").join(".")}</td>
